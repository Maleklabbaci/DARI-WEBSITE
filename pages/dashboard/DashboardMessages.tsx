      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-3">
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl overflow-hidden h-[750px] flex flex-col md:flex-row">
            {/* Inbox List */}
            <div className="w-full md:w-96 border-e border-slate-100 flex flex-col bg-slate-50/30">
              <div className="p-8 pb-4">
                <h2 className="text-2xl font-black text-slate-900 mb-6 tracking-tight">{t('dashboard.chat.title')}</h2>
                
                <div className="flex bg-slate-100 p-1.5 rounded-2xl mb-6">
                  <button 
                    onClick={() => setActiveTab('inbox')}
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === 'inbox' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                  >
                    <Inbox size={14} />
                    <span>{t('dashboard.chat.inbox')}</span>
                  </button>
                  <button 
                    onClick={() => setActiveTab('sent')}
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === 'sent' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                  >
                    <SendHorizontal size={14} />
                    <span>{t('dashboard.chat.sent')}</span>
                  </button>
                  <button 
                    onClick={() => setActiveTab('archived')}
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === 'archived' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                  >
                    <Archive size={14} />
                    <span>{t('dashboard.chat.archived')}</span>
                  </button>
                </div>

                <div className="relative">
                  <Search className={`absolute ${language === 'ar' ? 'right-4' : 'left-4'} top-3.5 text-slate-400`} size={16} />
                  <input 
                    type="text" 
                    placeholder={t('dashboard.chat.search')} 
                    className={`w-full ${language === 'ar' ? 'pr-11 pl-4' : 'pl-11 pr-4'} py-3.5 bg-white border border-slate-100 rounded-2xl text-xs outline-none focus:ring-2 focus:ring-blue-100 transition shadow-sm`} 
                  />
                </div>
              </div>
                ...
                {chats[activeTab].length > 0 ? (
                  chats[activeTab].map(chat => (
                    <button 
                      ...
                      className={`w-full p-5 text-start rounded-3xl border transition-all duration-300 relative group ${activeChat?.id === chat.id ? 'bg-white border-blue-100 shadow-lg ring-1 ring-blue-50/50 scale-[1.02]' : 'bg-transparent border-transparent hover:bg-white/60 hover:border-slate-100'}`}
                    >
                  ...
                  <div className="flex flex-col items-center justify-center py-20 text-slate-400 px-8 text-center">
                    ...
                    <p className="text-xs font-bold">{t('dashboard.chat.noMessages')}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="hidden md:flex flex-grow flex-col bg-white">
              {activeChat ? (
                  ...
                  <div className="flex-grow p-10 overflow-y-auto space-y-8 bg-slate-50/20">
                    <div className="flex flex-col items-center mb-8">
                       <div className="bg-orange-50 p-5 rounded-3xl border border-orange-100 flex items-start max-w-lg shadow-sm">
                        <ShieldAlert className="text-orange-600 me-4 mt-0.5 flex-shrink-0" size={20} />
                        <p className="text-xs text-orange-800 font-medium leading-relaxed">
                          <span className="font-black block mb-1">{t('dashboard.chat.safetyTitle')}</span>
                          {t('dashboard.chat.safetyDesc')}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-start">
                      <div className="flex items-end gap-3 max-w-[80%]">
                        ...
                        <div className={`bg-white p-5 rounded-[2rem] ${language === 'ar' ? 'rounded-br-none' : 'rounded-bl-none'} shadow-sm border border-slate-100`}>
                          ...
                          <p className="text-[10px] font-black text-slate-300 mt-3 text-end">14:30</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-end">
                      <div className="flex items-end gap-3 max-w-[80%] flex-row-reverse">
                        ...
                        <div className={`bg-blue-600 p-5 rounded-[2rem] ${language === 'ar' ? 'rounded-bl-none' : 'rounded-br-none'} shadow-xl shadow-blue-100`}>
                          ...
                          <p className="text-[10px] font-black text-blue-300 mt-3 text-end">14:35</p>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex-grow flex flex-col items-center justify-center text-slate-400 bg-slate-50/20">
                  ...
                  <h3 className="text-xl font-black text-slate-900 mb-2">{t('dashboard.chat.selectChat')}</h3>
                </div>
              )}
            </div>
          </div>
