      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-3 space-y-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <h1 className="text-3xl font-black uppercase tracking-tight">Analyses</h1>
              <p className="text-sm font-medium text-slate-500 mt-1">Suivez les performances de vos annonces et de vos boosts.</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
             {[
               ...
               { label: "N° affichés", val: "35", icon: <PhoneCall />, color: "text-green-600", bg: "bg-green-50" },
             ].map((stat, i) => (
          ...
          <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
            <div className="flex items-center justify-between mb-8 px-2">
              <h3 className="font-black text-slate-900 uppercase tracking-tight flex items-center">
                <BarChart3 size={20} className="mr-3 text-blue-600" /> Courbe de performance
              </h3>
            </div>
          ...
          <div className="space-y-6">
            <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">Détails par boost</h3>
                ...
                <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex items-center space-x-5">
                    ...
                    <div>
                      ...
                      <div className="flex items-center text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">
                        <Calendar size={12} className="mr-1.5" /> Du {boost.startDate} au {boost.endDate}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center space-x-3">
                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${boost.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
                      {boost.status === 'active' ? 'En cours' : 'Terminé'}
                    </span>
                    <div className="text-right">
                      <p className="text-[10px] font-black text-slate-400 uppercase">Budget investi</p>
                      <p className="text-sm font-black text-slate-900">{boost.budget.toLocaleString()} DA</p>
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8 p-8 bg-slate-50/50">
                  <div>
                    ...
                    <div className="mt-2 flex items-center text-[9px] font-bold text-blue-600 bg-blue-50 w-fit px-2 py-0.5 rounded">
                      <Users size={10} className="mr-1" /> {boost.estimatedReach[0]}-{boost.estimatedReach[1]} estimés
                    </div>
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">Taux de clic</p>
                    ...
                    <div className="mt-2 flex items-center text-[9px] font-bold text-green-600">
                      <ArrowUpRight size={12} className="mr-0.5" /> +2.4% vs Moyenne
                    </div>
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">Contacts reçus</p>
                  ...
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">Coût par contact</p>
                    <p className="text-xl font-black text-blue-600">{calculateCostPerContact(boost.budget, boost.results.messages + boost.results.phoneReveals)} <span className="text-[10px] text-slate-400">DA</span></p>
                  </div>
                </div>
              </div>
            ))}
