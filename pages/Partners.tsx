    ...
    <div className="bg-slate-50 min-h-screen py-24 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-20">
          ...
          <h1 className="text-4xl md:text-5xl font-black text-slate-900 mb-6 uppercase tracking-tight">Points de recharge & Partenariats</h1>
          <p className="text-xl text-slate-500 max-w-3xl mx-auto font-medium">Rechargez votre solde Dari facilement auprès de nos partenaires ou rejoignez notre réseau de distribution.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-24">
          <div className="bg-white p-12 rounded-[3.5rem] shadow-xl border border-slate-100 flex flex-col hover:shadow-2xl transition-all duration-500">
            ...
            <h2 className="text-3xl font-black mb-6 text-slate-900 tracking-tight">Trouver un point de recharge</h2>
            <p className="text-slate-500 mb-10 font-medium leading-relaxed">Achetez vos codes de recharge Dari directement dans nos boutiques partenaires et cybercafés à travers tout le pays (58 wilayas).</p>
            
            <div className="space-y-4 mb-12 flex-grow">
              {[
                { loc: "Alger - Bab Ezzouar (Cyber Immo)", dist: "2 km" },
                { loc: "Oran - Centre Ville (Boutique Dari)", dist: "Centre" },
                { loc: "Sétif - El Eulma (Multi-Services)", dist: "Marché" },
                { loc: "Constantine - Sidi Mabrouk", dist: "Station" }
              ].map((item, i) => (
            ...
            <button className="w-full py-5 bg-slate-900 text-white font-black rounded-2xl hover:bg-black transition shadow-xl shadow-slate-200 flex items-center justify-center active:scale-95">
              <MapIcon size={20} className="mr-3" /> Voir tous les points sur la carte
            </button>
          </div>

          <div className="bg-blue-600 p-12 rounded-[3.5rem] shadow-2xl text-white relative overflow-hidden flex flex-col">
            <div className="absolute top-0 right-0 w-80 h-80 bg-white opacity-5 rounded-full -translate-y-1/2 translate-x-1/2 blur-2xl"></div>
            <div className="relative z-10 flex flex-col h-full">
              ...
              <h2 className="text-3xl font-black mb-6 tracking-tight">Devenir point de recharge</h2>
              <p className="text-blue-100 mb-10 font-medium leading-relaxed">Boostez les revenus de votre commerce en devenant un distributeur officiel Dari. Simple, rapide et sans frais d'entrée.</p>
              
              <ul className="space-y-6 mb-12 flex-grow">
                {[
                  "Commission attractive sur chaque vente",
                  "Flux de clients additionnel garanti",
                  "Outil de gestion 100% numérique et sécurisé",
                  "Support dédié aux partenaires Dari"
                ].map((text, i) => (
                  <li key={i} className="flex items-center font-bold text-sm">
                    <div className="p-1 bg-white/20 rounded-lg mr-4 text-blue-100 shadow-sm"><BadgeCheck size={18} /></div>
                    {text}
                  </li>
                ))}
              </ul>
              <Link to="/contact" className="inline-block px-12 py-5 bg-white text-blue-600 font-black rounded-2xl hover:bg-blue-50 transition shadow-2xl text-center active:scale-95 uppercase tracking-widest text-sm">Rejoindre le réseau</Link>
            </div>
          </div>
        </div>

        <div className="bg-white rounded-[3rem] p-16 text-center border border-slate-100 shadow-sm">
          <h2 className="text-xl font-black mb-12 text-slate-400 uppercase tracking-[0.3em]">Nos Partenaires Bancaires & Réseaux</h2>
