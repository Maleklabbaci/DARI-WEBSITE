      ...
      <Link to={`/property/${property.id}`} className="block relative h-64 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex flex-wrap gap-2">
          ...
          {property.sellerType === 'agency' && (
            <span className="bg-slate-900/90 backdrop-blur-md text-white text-[10px] font-black uppercase px-3 py-1.5 rounded-xl shadow-sm tracking-widest">
              Pro
            </span>
          )}
        </div>
        ...
        <div className="absolute bottom-4 left-4">
          <div className="bg-slate-900/90 backdrop-blur-xl text-white px-5 py-2.5 rounded-[1.25rem] shadow-2xl border border-white/10">
            <span className="text-xl font-black">{property.price.toLocaleString('fr-DZ')}</span>
            <span className="text-sm font-bold ml-1">DA</span>
            {property.transaction === 'rent' && <span className="text-[10px] text-slate-400 font-bold ml-1 uppercase">/mois</span>}
          </div>
        </div>
      </Link>
      
      <div className="p-8 flex-grow flex flex-col">
        <div className="flex items-center text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4 space-x-2">
        ...
        <div className="mt-auto flex items-center justify-between pt-6 border-t border-slate-50">
          <div className="flex items-center space-x-5 text-slate-500">
            <div className="flex items-center font-bold text-xs">
              <Building size={16} className="mr-2 text-slate-300" />
              {property.rooms || '-'} <span className="hidden sm:inline ml-1">p.</span>
            </div>
            <div className="flex items-center font-bold text-xs">
              <Ruler size={16} className="mr-2 text-slate-300" />
              {property.surface} <span className="text-[10px] ml-1 uppercase">m²</span>
            </div>
          </div>
          <Link 
